...
uses
  ...
  uCarDetector, uSpyTaxiTypes, dxMapItem, uCoordinateGenerator, uCoordinateDownloader, uEventService
  //
...
type
  ...
  private
    ...
    FCoordinateGenerator: TCoordinateGenerator;
    FCoordinateUploader: TCoordinateDownloader;
...
procedure OnCarDetect(const ACar: TDriver);
...
begin
  ...
  frmMain.FCoordinateGenerator.AddDriver(ACar);
...
begin
  ...
  FCoordinateGenerator := TCoordinateGenerator.Create;

  FCoordinateUploader := TCoordinateDownloader.Create(constDBConnectionParams);

  FCoordinateUploader.OnLogMessage := FCommonEventService.LogMessageService;
  FCoordinateUploader.OnDownloadCooridnate := FCoordinateGenerator.AddPosition;

  FCoordinateUploader.DownloadCoordinate;
end;
...
begin
  ...
  if not FCoordinateGenerator.GenerateCoordinate(lZone) then
    Exit;
